...
const Animation = () => {
  ...
  const handlePrevImage = (itemIndex, totalImages) => {
    setCurrentImageIndex((prev) => ({
      ...prev,
      [itemIndex]:
        prev[itemIndex] > 0 ? prev[itemIndex] - 1 : totalImages - 1,
    }));
  };
  ...
  const handleNextImage = (itemIndex, totalImages) => {
    setCurrentImageIndex((prev) => ({
      ...prev,
      [itemIndex]:
        (prev[itemIndex] || 0) < totalImages - 1 ? (prev[itemIndex] || 0) + 1 : 0,
    }));
  };
                ...
                <div className="hidden lg:block">
                  <div className="grid grid-cols-3 gap-4">
                    {item.videoLinks.map((videoLink, i) => (
                      <iframe
                        key={i}
                        src={`https://player.vimeo.com/video/${videoLink?.split("/").pop()}`}
                        title={item.title || "Case Study"}
